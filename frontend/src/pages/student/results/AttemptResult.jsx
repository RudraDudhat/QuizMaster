...
export default function AttemptResult() {
    ...
    const stats = [
        { icon: <CheckCircle2 size={20} />, iconBg: '#ECFDF5', iconColor: 'var(--color-success)', value: result?.correctCount ?? 0, label: 'Correct answers', vc: 'var(--color-success)' },
        { icon: <XCircle size={20} />, iconBg: '#FEF2F2', iconColor: 'var(--color-danger)', value: result?.wrongCount ?? 0, label: 'Wrong answers', vc: 'var(--color-danger)' },
        { icon: <SkipForward size={20} />, iconBg: '#FFFBEB', iconColor: 'var(--color-warning)', value: result?.skippedCount ?? 0, label: 'Skipped', vc: 'var(--color-warning)' },
        { icon: <Plus size={20} />, iconBg: '#ECFDF5', iconColor: 'var(--color-success)', value: `+${result?.positiveMarks ?? 0}`, label: 'Marks earned', vc: 'var(--color-success)' },
        { icon: <Minus size={20} />, iconBg: '#FEF2F2', iconColor: 'var(--color-danger)', value: `-${result?.negativeMarksDeducted ?? 0}`, label: 'Marks deducted', vc: 'var(--color-danger)' },
        { icon: <Clock size={20} />, iconBg: '#EEF2FF', iconColor: 'var(--color-primary)', value: formatDuration(result?.timeTakenSeconds) || '—', label: 'Time taken', vc: 'var(--color-primary)' },
    ];
    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--color-bg-page)', paddingBottom: 48 }}>
            ...
            {bannerVisible && (
                <div style={{ background: '#FFFBEB', borderBottom: '1px solid #FDE68A', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <button onClick={() => setBannerVisible(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#92400E', display: 'flex', alignItems: 'center', padding: 4 }}>
                        <X size={16} />
                    </button>
                </div>
            )}
            ...
            <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 16px', display: 'flex', flexDirection: 'column', gap: 20 }}>
                ...
                {/* ── Hero Card ── */}
                <div style={{ background: '#fff', borderRadius: 20, border: '1px solid var(--color-border)', boxShadow: '0 4px 24px rgba(0,0,0,0.07)', padding: 'clamp(28px,5vw,48px) clamp(20px,5vw,40px)', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
                    ...
                    <div style={{ display: 'inline-flex', marginBottom: 24, animation: iconReady ? (isPassed ? 'popIn 0.6s cubic-bezier(0.34,1.56,0.64,1)' : 'wobble 0.7s ease, fadeUp 0.4s ease') : 'none', opacity: iconReady ? 1 : 0 }}>
                        <div style={{ width: 88, height: 88, borderRadius: '50%', background: isPassed ? '#ECFDF5' : '#FEF2F2', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 0 14px ${isPassed ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.08)'}` }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
                    {stats.map((s, i) => (
                        <div key={i} style={{ animation: `fadeUp 0.4s ease ${i * 70}ms both`, background: '#fff', borderRadius: 14, border: '1px solid var(--color-border)', boxShadow: 'var(--shadow-sm)', padding: '16px 14px' }}>
                ...
                {/* ── Performance Bars ── */}
                <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--color-border)', boxShadow: 'var(--shadow-sm)', padding: '24px' }}>
                    ...
                    <div style={{ marginBottom: 28 }}>
                        ...
                        <div style={{ height: 16, background: '#F3F4F6', borderRadius: 8, overflow: 'hidden', display: 'flex', gap: 2 }}>
                            {totalQ > 0 && <>
                                ...
                                <AnimatedBar pct={(result?.skippedCount ?? 0) / totalQ * 100} color="#D1D5DB" delay={240} />
                            </>}
                        </div>
                        <div style={{ display: 'flex', gap: 16, marginTop: 10, flexWrap: 'wrap' }}>
                            {[
                                ...
                                { color: '#D1D5DB', label: `Skipped (${result?.skippedCount ?? 0})` },
                            ].map(({ color, label }) => (
                    ...
                    <div>
                        ...
                        <div style={{ height: 16, background: '#F3F4F6', borderRadius: 8, overflow: 'visible', position: 'relative' }}>
                            ...
                            {passLinePct != null && (
                                <div style={{ position: 'absolute', top: -8, bottom: -8, left: `${Math.min(passLinePct, 99)}%`, width: 2, background: 'var(--color-danger)', borderRadius: 1, pointerEvents: 'none' }}>
                                    <div style={{ position: 'absolute', bottom: '100%', marginBottom: 4, left: '50%', transform: 'translateX(-50%)', whiteSpace: 'nowrap', fontSize: 10, color: 'var(--color-danger)', fontWeight: 700, background: '#fff', padding: '2px 5px', borderRadius: 4, border: '1px solid var(--color-danger)' }}>
                ...
                {/* ── Attempt Info ── */}
                <div style={{ background: '#fff', borderRadius: 16, border: '1px solid var(--color-border)', boxShadow: 'var(--shadow-sm)', padding: '20px 24px' }}>
